...
class EconBizIdFilterTest : DatabaseTest() {
    private val backend =
        LoriServerBackend(
            DatabaseConnector(
    ...
    private fun getInitialMetadata(): Map<ItemMetadata, List<ItemRight>> =
        mapOf(
            itemWithEconbizId to emptyList(),
        )
    ...
    fun testEconbizIdFilter() {
            ...
            runBlocking {
                backend.searchQuery(
                    ...
                    metadataSearchFilter =
                        listOf(
                            EconbizIDFilter(
                                econbizId = itemWithEconbizId.econbizId!!,
                            ),
                        ),
        ...
        assertThat(
            ...
            `is`(setOf(itemWithEconbizId)),
        )

        val searchResult2: SearchQueryResult =
            runBlocking {
                backend.searchQuery(
                    searchTerm = "${FilterType.ECONBIZID.keyAlias}:${itemWithEconbizId.econbizId}",
                    limit = 10,
                    offset = 0,
