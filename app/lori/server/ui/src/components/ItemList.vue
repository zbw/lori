...
export default defineComponent({
  ...
  setup() {
    ...
    const headers: ReadonlyDataTableHeader[] = [
      ...
      {
        title: "Zeitschriftentitel",
        sortable: true,
        value: "isPartOfJournal",
      },
      {
        title: "Zählung",
        sortable: true,
    ...
    onMounted(() => {
      ...
      if (!hasMetadataParameter && !hasInitSearch && !hasInitBookmarkId) {
        const queryParameterSearch = url.getQueryParameter(
          url.QUERY_PARAMETER_SEARCH,
        );
    ...
    const loadBookmarkView: () => boolean = () => {
      const bookmarkId: string | null = url.getQueryParameter(
    ...
    const loadGroupView: () => boolean = () => {
      const groupId: string | null = url.getQueryParameter(
      ...
      api
        .getGroupById(parseInt(groupId), undefined)
    ...
    const loadTemplateView: () => boolean = () => {
      const templateId: string | null = url.getQueryParameter(
    ...
    const getRightPP: () => string | null = () => {
      const rightId: string | null = url.getQueryParameter(
    ...
    const loadInitBookmarkId: () => boolean = () => {
      ...
      bookmarkApi
        .getBookmarkById(bookmarkIdParsed)
    ...
    const loadMetadataView: () => boolean = () => {
      const handle: string | null = url.getQueryParameter(
        url.QUERY_PARAMETER_HANDLE,
      );
    ...
    const loadInitSearchQuery: () => boolean = () => {
      const searchQuery: string | null = url.getQueryParameter(
      ...
      api
        ...
        .then((response: ItemInformation) => {
          if (response.accessStateWithCount != undefined) {
            searchStore.accessStateOnDateReceived =
    ...
    const processSearchResult = (response: ItemInformation) => {
      searchStore.filtersAsQuery =
        response.filtersAsQuery != undefined ? response.filtersAsQuery : "";
    ...
    const addRightSuccessful = (title: string, handle: string) => {
      successMsg.value =
        "Rechteinformation erfolgreich für Item " +
        "'" +
        title +
        " (" +
        handle +
    ...
    const selectedRowColor = (row: any) => {
      if (
        selectedItems.value[0] !== undefined &&
        selectedItems.value[0] == row.item.handle
      ) {
    ...
    const startExport = (format: ExportFormatRest) => {
      ...
      if (
        searchStore.filtersAsQuery.length > 0 &&
        searchStore.searchTerm.length > 0
      ) {
        exportSearchQuery =
          searchStore.filtersAsQuery + " & " + searchStore.searchTerm;
      } else {
        exportSearchQuery = searchStore.filtersAsQuery + searchStore.searchTerm;
      }
      jobApi
        .createJob(exportSearchQuery, format)
    ...
    const pollStatus = async (jobId: string) => {
      while (isPolling) {
        try {
          const response: JobStatusUpdateRest =
            await jobApi.getJobStatus(jobId);
...
<template>
  ...
  <v-main class="d-flex align-center justify-center">
    ...
    >
      <Dashboard v-on:dashboardClosed="closeDashboard"></Dashboard>
    </ResizableDialog>
    <ResizableDialog
      ...
      @close="closeDashboard"
    >
      <GroupOverview v-on:groupOverviewClosed="closeGroupDialog">
      </GroupOverview>
    </ResizableDialog>
    ...
    <v-card position="relative">
      ...
      <v-row no-gutters justify="space-around">
        <v-col> <b>Aktive Filter:</b> {{ searchStore.filtersAsQuery }} </v-col>
      </v-row>
      <v-row justify="end" align="center" class="ga-0 mb-3">
          ...
          <v-dialog v-model="searchHelpDialog" max-width="600px">
            ...
            <v-card>
              <template v-slot:actions>
                <v-spacer></v-spacer>
                <v-btn color="blue darken-1" @click="searchHelpDialog = false">
                  Zurück
                </v-btn>
              </template>
              <v-card-title class="text-h5">
              ...
              <v-card-text>
                <p class="text-left text-body-1 font-weight-bold">
                  Genereller Aufbau:
                </p>
                ...
                <table class="special">
                  ...
                  <tbody>
                    ...
                    <tr class="special">
                      <td class="special">
                        Lizenz URL ohne Protokolle, Punkte und Slashes
                      </td>
                ...
                <p class="text-left text-body-2 mt-1 mb-1">
                  Es ist möglich die verschiedenen Operatoren in einem Term zu
                  verwenden und mittels Klammern zu strukturieren.
                </p>
                ...
                <p class="text-left text-body-2 mt-1 mb-1">
                  Rechtstrunkierung ist möglich für alle Textbasierten Suchwerte
                  mit *, z.B:
                </p>
                <p class="text-center text-body-2 bg-grey-lighten-2 mt-1 mb-1">
                  lur:'by-nc-nd*'
                ...
                <p class="text-left text-body-1 mt-4 font-weight-bold">
                  Suche von mehreren Werten
                </p>

                <p class="text-left text-body-2 mt-1 mb-1">
                  Für folgende Suchschlüssel können mehrere Werte mit einem
                  Suchschlüssel eingegeben werden:
                  <b>pid</b
                  >,<b>hdl</b>,<b>isb</b>,<b>ppn</b>,<b>sig</b>,<b>zdb</b>.
                  <br />
                  <b>Wichtig</b>: Wildcards funktionieren nicht mit dieser
                  Syntax!
                </p>
        ...
        <v-col cols="auto">
          ...
          >
            <template v-if="exportDone"> Download </template>
        ...
        <v-col cols="auto">
          <v-btn
            ...
            :disabled="
              ...
              !searchStore.isLastSearchNonTrivialAndSuccessful
            "
          >
            Speichern
          </v-btn>
